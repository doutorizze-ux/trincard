-- Migração para sincronizar usuários com CPF temporário curto
-- Resolve problema de CPF muito longo
...
BEGIN
    ...
    IF user_count > 0 THEN
        FOR auth_user_record IN 
            ...
            WHERE pu.id IS NULL
        LOOP
            -- Gerar CPF temporário curto (máximo 14 caracteres)
            temp_cpf := 'TEMP' || LPAD(counter::text, 10, '0');
            ...
            RAISE NOTICE 'Sincronizado: % (%) - CPF: %', auth_user_record.email, auth_user_record.id, temp_cpf;
            counter := counter + 1;
        END LOOP;
    END IF;
